...
The MVP includes login, multiple roles, 2D clickable huts, mini simulations, quizzes, badges, a cultural passport, booking, and a marketplace path for traditional products.
...
| MVP Direction | Browser-based 2D cultural village with clickable huts, mini simulations, quizzes, badges, cultural passport, booking, and marketplace. |
...
Bootstrap will support a clean responsive UI. PostgreSQL will store users, huts, experiences, quizzes, progress, badges, bookings, products, and orders. JavaScript with SVG/Canvas will handle the actual interactive simulation layer.
...
- Track progress using quizzes, badges, and a digital cultural passport.
...
| Hut Experience Pages | Story, media, mini simulation, quiz, badge, and product suggestions. | High |
...
| Admin Dashboard | Manage users, huts, quizzes, products, bookings, and approvals. | High |
...
| Tourist / Learner | Explore culture, complete simulations, book experiences, and buy products. | Use virtual village, complete quizzes, earn badges, manage bookings, use cart and orders. |
...
| Admin | Manage daily operations and content quality. | Approve hosts, huts, products, bookings, quiz content, reviews, and reports. |
...
```text
Home -> Enter Virtual Village -> Click Hut -> Simulation + Quiz -> Badge / Passport -> Marketplace
```
...
4. User reads the story, views images/audio/video, completes the mini simulation, and answers a quiz.
...
| Hut Experience Page | Story, media, simulation, quiz, badge, marketplace suggestions. | Complete Hut |
...
| Admin Dashboard | Approvals, content, users, quizzes, analytics, reports. | Approve / Manage |
...
| Database | PostgreSQL | Users, roles, huts, stories, quizzes, badges, progress, bookings, products, carts, orders, reviews. |
...
| Admin | Django Admin + Custom Admin Pages | Fast content management for huts, quizzes, products, bookings, and approvals. |
...
| Task | Handled By |
|---|---|
| Store hut data, story text, quiz questions, and media paths | Django + PostgreSQL |
...
| Validate quiz answers and save progress | Django backend + PostgreSQL |
...
| Cooking Hut | Select correct ingredients for a traditional Jaffna dish. | Ingredient cards with checkbox/tile selection. | Correct ingredients, quiz score. |
| Fishing Life Hut | Match fishing tools with their purpose. | Drag-and-match or dropdown matching. | Matching score, completion. |
| Folk Music Hut | Listen to a sound and identify the instrument or rhythm. | HTML audio + quiz choices. | Answer score, badge. |

## Simulation Acceptance Criteria

- Each hut must include a story section, media section, one mini simulation, one quiz, and one badge.
...
| FR-04 | Hut Experience | Each hut must show story, media, activity, quiz, badge, and product recommendations. | Must |
| FR-05 | Mini Simulation | Each hut must include one JavaScript-based simulation activity. | Must |
| FR-06 | Quiz System | System must store questions, options, correct answers, and user scores. | Must |
| FR-07 | Progress Tracking | System must save hut completion, quiz score, badge status, and completion time. | Must |
...
| FR-12 | Admin Approval | Admins must approve hosts, huts, experiences, quizzes, products, and reviews. | Must |
| FR-13 | Content Management | Admins must manage hut content, stories, media, quizzes, and badges. | Must |
...
| Reliability | Quiz scores, progress, cart, bookings, and orders must not be lost unexpectedly. |
...
- Five hut pages:
  ...
  - Fishing Life
  - Folk Music
- One mini simulation and one quiz per hut.
- Badge unlocking and digital cultural passport progress tracking.
- Basic Django admin/custom admin dashboard for hut, quiz, badge, product, and user management.
...
| QuizQuestion | hut, question, options, correct_answer, explanation | Quiz system. |
...
| 4. Hut + Simulation | Weeks 4-5 | Build hut pages, mini simulations, quiz system, progress saving, and badges. | Functional simulation flow. |
| 5. Marketplace + Booking | Week 6 | Build product listing, cart, order placement, simple booking path. | End-to-end product path. |
| 6. Admin | Week 7 | Configure Django admin/custom admin for content, hosts, products, quizzes, and approvals. | Admin management system. |
...
| Users see it only as a game. | Use storytelling, cultural explanations, quizzes, badges, and cultural passport framing. |
...
| simulations | Simulation configuration, quiz handling, scoring, progress, badges, cultural passport. |
